...
export default function RunDetail() {
  ...
  const regressedCount = run.pages.filter((p) => p.diff_score > 0.15).length
  ...
  const diffColor = (score) => {
    if (score < 0.05) return 'text-emerald-600 bg-emerald-50'
    if (score < 0.2) return 'text-amber-600 bg-amber-50'
    return 'text-red-600 bg-red-50'
  }
  const diffBg = (score) => {
    if (score < 0.05) return 'border-emerald-200'
    if (score < 0.2) return 'border-amber-200'
    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 mt-2 mb-6">
        <div className="flex flex-wrap gap-2 mb-3">
          ...
          <span className={`text-xs px-2 py-0.5 rounded-full ${run.environment === 'staging' ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
            {run.environment}
          </span>
        ...
        <button onClick={downloadReport} className="mt-4 text-sm px-4 py-2 bg-gray-100 rounded-lg hover:bg-gray-200">📥 Download Report (JSON)</button>
      </div>
        ...
        {run.run_type === 'rerun' && (
          <div className="flex gap-1">
            {[
              ...
              { key: 'changed', label: 'Changes Only', count: run.pages.filter((p) => p.diff_score !== null && p.diff_score > 0).length },
              { key: 'regressed', label: 'Regressions', count: run.pages.filter((p) => p.diff_score > 0.15).length },
      ...
      <div className="space-y-3">
            ...
            <div key={i} className={`bg-white rounded-xl border shadow-sm overflow-hidden ${p.diff_score !== null ? diffBg(p.diff_score) : 'border-gray-200'}`}>
              ...
              {isExpanded && (
                <div className="border-t border-gray-100 p-4 bg-gray-50 space-y-4">
                  {/* Screenshots — full width, click to zoom */}
                  <div>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div>
                        ...
                        <img
                          ...
                          onClick={(e) => { e.stopPropagation(); setLightboxImg(p.screenshot_url) }}
                        />
                      </div>
                      {p.diff_image_url && (
                        <div>
                          <p className="text-xs text-gray-400 mb-1">Baseline (for comparison)</p>
                          <img
                            src={p.diff_image_url}
                            alt="Baseline"
                            className="w-full rounded-lg border border-red-200 cursor-pointer hover:opacity-90 transition"
                            onClick={(e) => { e.stopPropagation(); setLightboxImg(p.diff_image_url) }}
                          />
                        </div>
                      )}
                    </div>
                  ...
                  {p.ecommerce_checks && (
                    <div>
                      <h4 className="text-xs font-semibold text-gray-500 uppercase mb-2">E-commerce Checks</h4>
                      <div className="flex gap-2 flex-wrap">
                        {Object.entries(p.ecommerce_checks).map(([k, v]) => (
                          <span key={k} className={`text-xs px-2 py-1 rounded-lg ${v === 'pass' ? 'bg-emerald-100 text-emerald-700' : v === 'fail' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-500'}`}>
                            {k.replace(/_/g, ' ')}: {v}
                          </span>
                        ))}
                      </div>
        ...
        {filter !== 'all' && run.pages.filter((p) => {
          ...
          <div className="text-center py-10 text-gray-400">
            ...
            <p className="text-sm">{filter === 'changed' ? 'No visual changes detected — all pages match the baseline.' : filter === 'regressed' ? 'No regressions found — all differences are within tolerance.' : 'No errors found — all pages loaded successfully.'}</p>
          </div>
        )}
      </div>
